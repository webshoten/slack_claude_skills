...
use セッションの終了時に学習提案を行う。終了とみなすタイミング:

- ユーザーが `train` コマンドでモードを切り替えたとき
- ユーザーが別のスキルで `use` を開始したとき

#### フロー

```
1. use セッション終了を検知
2. スレッドの会話履歴（use 中のやりとり）を取得
3. Claude（Haiku）に会話を要約させ、スキルに追加すべき知見を抽出
   - 成功したパターン（例: SUUMOの検索URLの組み立て方）
   - 学んだ制約（例: 特定サイトはbotブロックされる）
   - ユーザーが教えてくれたドメイン知識
4. 知見がある場合 → train の仕組みを使って diff + OK/NG ボタンを表示
5. ユーザーが OK → スキルに反映（既存の handleTrainConfirm を流用）
6. ユーザーが NG → 破棄
7. 知見が特にない場合 → 何もしない（ノイズを出さない）
```
...
追加すべき知見の例:
- 成功した手順やパターン（URL構造、API仕様など）
- 発見した制約や注意点
- ユーザーが教えてくれたドメイン知識

既にスキルに含まれている内容は提案しないでください。
些細な内容や一回限りの情報は含めないでください。
...
- Haiku で抽出（コスト抑制）
- 知見がない場合は沈黙（毎回提案するとうるさい）
- use セッションの会話履歴が短い（2往復以下）場合はスキップ
