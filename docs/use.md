...
### Step 4: スレッド内でのモード切り替え（一部実装済み）
- train コマンド実行時に use_sessions を消す処理を app.ts に追加（実装済み）
- `handleUseInThread` — 同じスキル（すでにモード）/ 別のスキル（上書き）/ train からの切り替え（未実装）
- use → train → use の交互切り替えが正しく動くことを確認

### Step 5: use 会話からのスキル学習提案（未実装）

use モードでの会話から得られた知見を、自動的にスキルへフィードバックする仕組み。

#### トリガー

**毎回の use 応答の直後**に学習抽出を実行する。

#### フロー

```
1. use の通常応答をスレッドに投稿（既存の処理）
2. 応答後、同じ会話履歴 + Claude の回答を Haiku に渡して知見を抽出
3. 知見がある場合 → diff + OK/NG ボタンを表示
4. ユーザーが OK → スキルに反映（既存の handleTrainConfirm を流用）
5. ユーザーが NG → 破棄
6. 知見がない場合 → 何もしない（ノイズを出さない）
```

#### 実装の流れ（handleUseMessage 内）

```typescript
// 1. 通常の use 応答（既存）
const response = await llm.chat(apiKey, messages, systemPrompt, "claude-sonnet-4-6", USE_TOOLS);
await messenger.replyInThread(channel, threadTs, response);

// 2. 学習抽出（応答後に非同期で実行）
const skillContent = await skillStore.get(session.skillName);
const learnPrompt = buildLearnSystemPrompt(skillContent, messages, response);
const learnResult = await llm.chat(apiKey, [{ role: "user", content: learnPrompt }], "", "claude-haiku-4-5-20251001");
const parsed = JSON.parse(learnResult);

// 3. 知見があれば提案
if (parsed.type === "proposal") {
  await pendingStore.save(threadTs, parsed.updated);
  await messenger.replyInThread(channel, threadTs, parsed.diff, confirmBlocks);
}
```
...
```
以下は use モードでの会話履歴と、それに対する AI の回答です。
この会話から、スキルに追加すべき知見があれば提案してください。

追加すべき知見の例:
- 成功した手順やパターン（URL構造、API の使い方など）
- 発見した制約や注意点（bot ブロック、レートリミット等）
- ユーザーが教えてくれたドメイン知識
...
## 会話履歴
{Slack から取得した use セッションの会話（ツール中間結果は含まない）}
...
- `{"type":"none"}` — 追加すべき知見がない場合

#### 表示

知見がある場合のみ表示:
```
この会話から学んだことをスキルに追加しませんか？
...
- 既存の train の仕組み（PendingStore + OK/NG ボタン + handleTrainConfirm）をそのまま流用
  - action_id は `train_ok` / `train_ng` を共用
  - handleTrainConfirm は PendingStore → SkillStore に保存するだけなので use/train を区別しない
- Haiku で抽出（コスト抑制、Sonnet は不要）
- 会話履歴は Slack から取得したテキストのみ（ツール呼び出しの中間結果は含まない）
- 知見がない場合は沈黙（毎回提案が出るわけではない）
